...
def update_inventory_chart(request):
    # Kết nối với cơ sở dữ liệu SQLite
    conn = sqlite3.connect('db.sqlite3')
    ...
    # Lưu biểu đồ thành file hình ảnh, ghi đè lên ảnh cũ
    output_path = 'static/admin/inventory_optimization.png'
    if not os.path.exists('static/admin'):
        os.makedirs('static/admin')
    plt.savefig(output_path)
    plt.close()

    return redirect('inventory-chart')

def inventory_chart_view(request):
    return render(request, 'admin/demand_forecast.html')